...
| 버전 문자열 | ✅ `0.1.0` |
| 기존 월드에서 로드 | ✅ 사람의 본 월드(접속 61회·908청크)에 몇 주째 얹어 불러왔다 |
| 멀티플레이 | ✅ 데디서버 + 클라이언트로 자가 테스트를 여러 판 돌렸다 |
| 사람 둘이 동시 접속 | ⚠ 안 해봤다. 다만 아래를 볼 것 |
...
- [x] `status.md` 체크포인트 — 자원 복제(1번)를 포함해 인게임에서 확인했다
...
> **이 목록이 한동안 틀려 있었다.** 위 세 줄은 증거가 쌓인 뒤에도 갱신되지 않아 몇 주 동안
> "미검증"으로 남아 있었고, 그 때문에 배포가 실제보다 멀어 보였다. **체크리스트는 일이 끝날
> 때마다 갱신한다.** 안 그러면 목록이 현실이 아니라 과거를 가리킨다.

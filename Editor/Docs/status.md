...
3. **작업대를 설치하고 E로 열리는가** ← **여기가 이번 작업에서 제일 불확실한 지점.**
   UnityEvent 배선(`m_TargetAssemblyTypeName` 문자열)은 `preflight.py`가 못 잡는다
4. **작업대에서 파일런이 제작되는가**
5. **보이는가** — 스프라이트가 안 보이면 `research.md` 12장의 프리팹 참조 형태를 의심한다.
   `m_address`는 이제 자동 검사 대상이라 용의선상에서 내려갔다.
   **파일런이 2×2, 작업대가 4×2 타일로 보이는 것이 정상이다** — 아래 참조
6. **범위가 맞는가** — 파일런 기준 10타일 안 상자는 안 부서지고 **밖 상자는 부서지는지**
7. **나중에 놓은 파일런도 듣는가** — 상자를 먼저 놓고 그 옆에 파일런을 설치.
   `[NoBreakZone] pylons changed` 로그가 뜨고 그 상자가 보호되면 재판정이 도는 것이다
8. **파일런을 다시 회수할 수 있는가** — 3단계에서는 파일런이 자기 자신을 보호하지 않게 해뒀다.
   4단계에서 반대가 된다

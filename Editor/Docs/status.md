...
- `Scripts/Converters/NoBreakZoneWorkbenchRecipeInjectionConverter.cs` — 철 작업대에 레시피 주입.
...
빌드 없이 밀어둔 분량이 크므로 **첫 빌드에서 컴파일 에러가 여러 개 나오는 것을 정상으로 본다.**
커밋이 단계별로 끊겨 있어 범위는 좁다.
...
2. **철 작업대(IronWorkBench)에 `Pylon Workbench` 레시피가 뜨는가** — 레시피 주입이 먹었는지.
...
22. 파일런이 여러 개 보일 때 **눈에 띄게 버벅이지 않는가** (파일런당 80칸)
...
### 파일런을 꺼내는 경로 — 해결 (2026-08-05)

콘솔 명령이 필요 없다. **작업대를 6단계에서 앞당겨 만들었다.**

```
철 작업대(바닐라)  ──레시피 주입──▶  Pylon Workbench  ──CraftingAuthoring──▶  파일런
```

레시피 주입이 없으면 이 모드는 인게임에서 **아예 도달할 수 없다.** 철 작업대는 게임 것이라
...
| 한 것 | 어디 |
| --- | --- |
| 경계 타일 계산 (반지름 10 → 80칸, 모서리 중복 없음) | `Logic/NoBreakZoneRange.WriteBoundaryTiles` + 테스트 |

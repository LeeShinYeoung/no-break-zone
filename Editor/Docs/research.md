...
→ **뒤집힘:** 컬러로 직접 칠한다. 그레이스케일은 스킨용 선택 기능이었다 (31-3).
...
**당시 진행 기록에 적었던 스프라이트 결정 (a)는 이유가 틀렸다** (맞는 판단은 바로 아래 결말). "`spritePixelsToUnits`를 32로 두면 1타일로
...
### 어느 슬롯이 어느 언어인가 — 한국어는 7번 (2026-09-13 확정)

13개 주소는 게임 번들 안 `LanguageDataBlock` 에셋의 guid다. 처음에는 못 찾았다.

- 13개를 전부 guid로 환원해 `ck-db`·`ck-mods`·CoreLib·SDK 예제를 검색 → **0건**
- 레퍼런스 모드 중 **비영어로 현지화한 것이 없다**

**풀린 방법:** 게임 번들(`defaultlocalgroup_assets_all.bundle`)을 풀어 13개 `LanguageDataBlock`을
직접 읽었다. 유니티 GUI는 필요 없었다. 슬롯 순서는 **에셋 이름의 알파벳순**이고, 그래서 영어가 0이
아니라 2다.

| 슬롯 | 언어 | 슬롯 | 언어 |
| --- | --- | --- | --- |
| 0 | 중국어 간체 (게임 표기 `zh-ch`) | 7 | **한국어** |
| 1 | 중국어 번체 | 8 | 포르투갈어(브라질) |
| 2 | **영어** — 기본 언어, 다른 슬롯이 비면 이걸 쓴다 | 9 | 러시아어 |
| 3 | 프랑스어 | 10 | 스페인어 |
| 4 | 독일어 | 11 | 태국어 |
| 5 | 이탈리아어 | 12 | 우크라이나어 |
| 6 | 일본어 | | |

영어 2번은 따로도 맞는다 — 게임의 `PRIMARY_LANGUAGE_INDEX`가 2이고, SDK 예제 `ItemExample/Sword1.asset`은
딱 한 슬롯(2번)만 "The Greatest Sword"로 채워져 있다. 13개 전부가 `genassets.py`의 `LANGUAGE_SLOTS`에
있으므로, 새 번역은 스펙의 `localized`에 언어 코드 하나를 더하면 들어간다.
...
빌드를 돌렸고, 그것이 비정상 종료로 이어진 것으로 보인다. workflow.md "루프" 절에 규칙으로 박았다.
...
## 31. 옛 진행 기록에서 옮긴 판단 근거 (2026-09-14 정리)

`status.md`의 옛 기록을 지우면서 옮긴 것 셋. 코드 주석에는 남아 있었지만 문서에는 이곳에만 있다.

### 31-1. 타일은 "한 번만 판정"하면 안 된다 (2026-09-09)
...
하고 빼앗는 경로가 없기 때문이다. ECS 하네스에서 수정 전 빨강(39/1), 수정 후 초록(40/0).

게임에서 본 `ore-inside-still-breaks` 실패는 결국 다른 원인이었다 — 곡괭이 광석은 벽 안에 있다(23장).
이 구멍은 그것과 별개로 진짜다.

### 31-2. 파괴 방지에 체력 바닥선을 고른 이유 (2026-09-09)

보호가 `DontDestroyOnZeroHealthCD` 하나로 **파괴만** 막던 때, 보호받는 물건은 체력 0에 앉아 있다가
파일런을 끄는 순간 한꺼번에 터졌다. 게임은 체력을 깎는 것과 파괴하는 것을 따로 하기 때문이다.
`NoBreakZoneHealthFloorSystem`이 보호 대상이 체력 0인 채로 파괴 관문에 닿지 못하게 한다 —
`UpdateHealthSystemGroup` 뒤, `SetEntitiesDestroyedSystem` 앞이라는 단 하나의 자리이고, 그 순서는
`VerifySystemOrder`가 못박는다.

- 파괴 버퍼를 거르는 대신 체력 바닥을 택한 이유는 **누가 죽이려는지 몰라도 되기 때문**이다.
  무엇이 죽이려 하든 그 관문을 지나야 하고, 그때는 조건이 이미 거짓이다
- 1이 아니라 `maxHealth`로 되돌린다. 1로 두면 같은 버그의 약한 형태가 된다 — 체력 1짜리 기지는
  폭발 한 방에 사라진다
- `DropsLootWhenDamagedCD`를 가진 것은 되돌리지 않는다. 지금은 닿을 수 없는 조건이지만, 체력을
  되돌리는 구조 자체가 복제 위험을 키우므로 이중으로 막는다

### 31-3. 스프라이트는 컬러로 직접 칠해도 된다 (2026-08-06)

그레이스케일 + `GradientMapDataBlock`은 스킨(색 교체)용 선택 기능이다. 레퍼런스 모드는 컬러로 직접
그린다. 그래서 `genassets.py`는 그라디언트 맵 칸을 비워 두고 PNG의 색을 그대로 쓴다. 8장 끝의
"그레이스케일 방식을 쓴다"는 이걸 알기 전의 계획이다.

## 7. 열린 질문 / 다음 검증

- [x] ~~로컬 모드 활성화 절차~~ — 따로 켤 필요가 없다. `build.ps1`이 Mods 폴더에 넣으면 다음 실행에
  자동으로 올라온다 (`Player.log`의 `[NoBreakZone] loaded`)
- [ ] 재적용에 전체 재시작이 필요한지, 부분 재로드로 되는지
- [x] ~~배포(mod.io/창작마당) 절차~~ — SDK 업로드 탭을 디컴파일해 확인했다. `store.md` (2026-09-13)
